...
`@part1`
The break statement is used to exit the current loop before its normal ending. In nested loops, break allows for specification of which loop to exit. See the Bash info pages for more.
```{r}
for (( i = 1; i < 5; i++ ))
do
  echo "Outer loop: $i"
