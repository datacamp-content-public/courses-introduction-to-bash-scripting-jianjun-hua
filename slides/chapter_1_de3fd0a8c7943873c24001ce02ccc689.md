...
`@part1`
The continue statement (continue [n])tells Bash to skip the remaining of the current iteration and begin the next one. If n is specified, resumes the nth enclosing loop.
```
for (( i = 1; i <= 10; i++)); do
  if (( $i >= 2)) && (( $i <= 8)); then
    continue
...
`@script`
The continue statement tells Bash to stop running through the current iteration of the loop and begin the next one. 
